...
import re
c=[]
for d,f,a,b in(re.match('([xy])=(\d+).*?(\d+)..(\d+)',x).groups()for x in open("input.txt")):f=int(f);c+=[(x,f)if d>'x'else(f,x)for x in _(int(a),int(b)+1)]
x,y=(min(z[i]for z in c)for i in(0,1))
w,h=(max(z[i]for z in c)-(x,y)[i]for i in(0,1))
m=[[0]*(w+2)for i in _(h+1)]
...
def D(x,y):
 ...
 while h>y>=s and m[y+1][x]&1:
  ...
  if a+b>1:
   for f in _(l,r+1):m[y][f]=3
  a or D(l,y);b or D(r,y);y-=1
D(500-x,0)
S=sum
print S(S(x/2for x in r)for r in m)
print S(S(x>2for x in r)for r in m)
