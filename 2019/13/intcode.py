...
Includes puzzle solutions up to: 13.2 (day.part)
"""
import sys
...
Puzzles = []
def puzzle(fn):
    Puzzles.append((fn.__name__.strip('_').replace('_', '.', 1).replace('_', '-'), fn))
    return fn
...
def _13_2(vis=False):
    ...
    while True:
        try:
            ...
            if vis:
                if x < 0:
                    sys.stdout.write("\x1b[22;1H{}\x1b[K".format(t))
                else:
                    sys.stdout.write("\x1b[{};{}H{}".format(y+1, 2*x+1, "  ##[]==()"[t*2:t*2+2]))
                sys.stdout.flush()
